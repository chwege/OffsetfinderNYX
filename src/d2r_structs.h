...
inline RetCheckDataV2* kCheckDataV2;

struct RetCheckRuntimeV2 {
  const uint8_t* protected_code_begin;
  const uint8_t* protected_code_end;
  uint32_t protected_code_size;
  bool entry_contract_ok;
  bool request_contract_ok;
  bool dispatcher_contract_ok;
  void* request_allocator;
  void* request_submit;
  void** dispatcher_slot;
  void* dispatcher_object;
  void* dispatcher_method;
  uint32_t dispatcher_method_offset;
};
inline RetCheckRuntimeV2 kCheckRuntimeV2;
...
constexpr size_t kUnitTypeCount = 6;
typedef D2UnitStrc* EntityHashTable[kUnitHashTableCount];
inline EntityHashTable* sgptClientSideUnitHashTable;

// Optional globals used to verify patch-resilient patterns against externally
// confirmed current-build RVAs. They are never populated from static RVAs.
inline uint8_t* Reference_UIOffset;
inline uint8_t* Reference_Expansion;
inline uint8_t* Reference_Roster;
