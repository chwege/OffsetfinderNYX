...
static void* D2Alloc(size_t size, size_t align = 0x10) {
  ...
  return alloc_fn(allocator, size, align);
}
...
#pragma pack(pop)
  ...
  D2AutomapCellStrc* new_cell = static_cast<D2AutomapCellStrc*>(D2Alloc(sizeof(D2AutomapCellStrc)));
  if (new_cell == nullptr) {
    PIPE_LOG("Failed to allocate automap cell link");
    return;
  }
