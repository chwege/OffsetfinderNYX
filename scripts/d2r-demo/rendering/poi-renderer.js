...
import { projectClient } from './local-automap.js';
...
export function redrawPois(pois, me, exitKeys, levelId, rebuildDiag) {
  ...
  // Convert client coordinates into the fixed level overview.
  // Projection is local and does not call a protected game function.
  const results = [];
  const player = me.path
    ? projectClient(me.path.clientCoordX, me.path.clientCoordY)
    : null;
  const playerSX = player?.x;
  const playerSY = player?.y;
  for (let i = 0; i < pois.length; i++) {
    const p = pois[i];
    const screen = projectClient(p.clientX, p.clientY);
    if (!screen) continue;
