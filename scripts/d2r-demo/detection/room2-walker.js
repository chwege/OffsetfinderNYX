 ...
 *   rooms:             Array<{minX, minY, maxX, maxY}>,
...
export function walkRoom2(lvlAddr, currentLevelId, allAdjSet) {
  ...
  if (!lvlAddr || lvlAddr === 0n) {
    diag = ' noLvlAddr';
    return { centerWarps, adjLevelCoords, curLevelBounds, rooms, roomTileExits,
  ...
  try {
    ...
    while (room2Ptr && room2Ptr !== 0n && room2Ptr > 0x10000n
      ...
      try {
        ...
        const nextRoom = rDv.getBigUint64(0, true);
        const roomCoordsValid = rtX >= 0 && rtY >= 0 && rtX <= 200000 && rtY <= 200000
          && rsX > 0 && rsY > 0 && rsX <= 512 && rsY <= 512;
        if (!roomCoordsValid) {
          diag += ` invalidRoomCoords=${rtX},${rtY},${rsX},${rsY}`;
          room2Ptr = nextRoom;
          continue;
        }
        rooms.push({
  ...
  return {
    centerWarps, adjLevelCoords, curLevelBounds, rooms, roomTileExits,
    presetBosses, presetWaypoints, presetNPCs, presetCairnStones,
    realTombLevel, diag,
  };
}
