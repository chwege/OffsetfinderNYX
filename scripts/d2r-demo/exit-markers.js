...
import {
  clearLocalAutomap,
  configureLocalAutomap,
  viewportChanged,
} from './rendering/local-automap.js';
import { clearRooms, redrawRooms } from './rendering/room-renderer.js';
...
export class ExitMarkers {
  constructor(objMgr, options = {}) {
    ...
    this._periodicPerfEnabled = perf.periodic !== false;
  ...
  tick() {
    try {
      ...
      if (!me) {
        const now = Date.now();
        if (now - this._lastWaitingLog >= 2000) {
          this._lastWaitingLog = now;
          const players = this._objMgr.getUnits(0)?.size || 0;
          const monsters = this._objMgr.getUnits(1)?.size || 0;
          this._appendPerfLog(
            `${new Date(now).toISOString()} map_waiting reason=no_local_player`
            + ` local_id=${this._objMgr.localPlayerId} players=${players}`
            + ` monsters=${monsters} scan_cycles=${this._objMgr.incrementalCycleCount}`
            + ` scan_error=${this._formatLogValue(this._objMgr.lastReadOnlyScanError || 'none')}`,
          );
        }
        this._clearAll();
        return;
      ...
      if (viewportChanged() && this._levelBounds) {
        configureLocalAutomap(this._levelBounds);
        redrawRooms(this._rooms, this._roomKeys);
        this._lastRedraw = 0;
      }
  ...
  _rebuild(me, currentLevelId, reason = 'unknown') {
    ...
    const rooms = [];              // cached current-level Room2 rectangles
    ...
    if (locked !== true) {
      ...
      this._appendPerfLog(
        `${new Date().toISOString()} map_rebuild level=${currentLevelId}`
        + ` reason=${reason} result=lock_timeout duration_ms=${Date.now() - rebuildStart}`,
      );
      return;
    }
    ...
    this._rooms = rooms;
    this._levelBounds = curLevelBounds;
    this._needsBossRescan = built.needsBossRescan;
    if (built.realTombLevel > 0) this._realTombLevel = built.realTombLevel;
    rd += built.diag;
    const projection = configureLocalAutomap(curLevelBounds);
    if (projection) {
      const roomsDrawn = redrawRooms(rooms, this._roomKeys);
      rd += ` localMap=ready rooms=${roomsDrawn} scale=${projection.scale.toFixed(4)}`;
    } else {
      clearRooms(this._roomKeys);
      rd += ' localMap=unavailable';
    }
    const boundsText = curLevelBounds
      ? `${curLevelBounds.minX},${curLevelBounds.minY}-${curLevelBounds.maxX},${curLevelBounds.maxY}`
      : 'none';
    this._appendPerfLog(
      `${new Date().toISOString()} map_rebuild level=${currentLevelId}`
      + ` reason=${reason} result=${projection ? 'ready' : 'unavailable'}`
      + ` rooms=${rooms.length} pois=${this._pois.length} bounds=${boundsText}`
      + ` scale=${projection ? projection.scale.toFixed(4) : 'none'}`
