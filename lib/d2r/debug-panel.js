...
const MAX_VISIBLE_NODES_PER_TYPE = 100;
...
class DebugPanel {
  constructor(objectManager, options = {}) {
    ...
    this._headerWasOpen = new Array(TYPE_COUNT).fill(false);
    for (let i = 0; i < TYPE_COUNT; i++) {
  ...
  _onUnitAdded(unit, type) {
    ...
    if (this._isHeaderOpen(type)) this._addNode(unit, type);
  }

  _addNode(unit, type) {
    if (this._nodes[type].has(unit.id) ||
        this._nodes[type].size >= MAX_VISIBLE_NODES_PER_TYPE) return;
    const node = new gui.TreeNode(this._unitLabel(unit));
    const detail = new gui.Text(this._unitDetail(unit, type));
    node.add(detail);
    node._detail = detail;
    node._unit = unit;
    node._labelText = this._unitLabel(unit);
    node._detailText = this._unitDetail(unit, type);
  ...
  _updateHeaderLabel(type) {
    const total = this._objMgr.getUnits(type)?.size ?? 0;
    const shown = this._nodes[type].size;
    const suffix = shown < total && this._isHeaderOpen(type) ? `, showing ${shown}` : '';
    this._headers[type].label = `${TYPE_NAMES[type]} (${total}${suffix})###DebugPanelHeader${type}`;
  }

  _isHeaderOpen(type) {
    try {
      return !!(this._headers[type] && this._headers[type].open);
    } catch (_) {
      return false;
    }
  }
  ...
  refresh() {
    ...
    for (let type = 0; type < TYPE_COUNT; type++) {
      ...
      for (const [id, node] of this._nodes[type]) {
        ...
        const labelText = this._unitLabel(unit);
        const detailText = this._unitDetail(unit, type);
        if (labelText !== node._labelText) {
          node.label = labelText;
          node._labelText = labelText;
          nodeUpdates++;
        }
        if (detailText !== node._detailText) {
          node._detail.text = detailText;
          node._detailText = detailText;
          nodeUpdates++;
        }
      }
    }
  ...
  _maybeEmitPerf(now) {
    ...
    const unitCounts = TYPE_NAMES.map((_, type) => this._objMgr.getUnits(type)?.size ?? 0);

    this._logger(
      `[UnitExplorerPerf] open=${panelOpen ? 1 : 0} visible=${panelVisible ? 1 : 0} visible_known=${panelVisibleKnown ? 1 : 0}` +
      ` tick_ms=${(this._objMgr.tickTimeNs / 1000000).toFixed(2)}` +
      ` lock_ms=${(this._objMgr.gameLockTimeNs / 1000000).toFixed(2)}` +
      ` nodes=${totalNodes} headers_open=${openHeaders}/${TYPE_COUNT}` +
      ` players=${unitCounts[UnitTypes.Player]} monsters=${unitCounts[UnitTypes.Monster]}` +
      ` objects=${unitCounts[UnitTypes.Object]} missiles=${unitCounts[UnitTypes.Missile]}` +
      ` items=${unitCounts[UnitTypes.Item]} tiles=${unitCounts[UnitTypes.Tile]}` +
  ...
  _clearNodes() {
    for (let type = 0; type < TYPE_COUNT; type++) {
      this._clearTypeNodes(type);
    }
  }

  _clearTypeNodes(type) {
  ...
  _resyncType(type) {
    ...
    if (units) {
      for (const [, unit] of units) {
        ...
        if (this._nodes[type].size >= MAX_VISIBLE_NODES_PER_TYPE) break;
      }
    }
    this._updateHeaderLabel(type);
  }
  ...
  _syncHeaderVisibility() {
    for (let type = 0; type < TYPE_COUNT; type++) {
      const open = this._isHeaderOpen(type);
      if (open && !this._headerWasOpen[type]) this._resyncType(type);
      if (!open && this._headerWasOpen[type]) this._clearTypeNodes(type);
      this._headerWasOpen[type] = open;
      this._updateHeaderLabel(type);
    }
  }
  ...
  _resyncNodes() {
    this._clearNodes();
    for (let type = 0; type < TYPE_COUNT; type++) {
      if (this._isHeaderOpen(type)) this._resyncType(type);
      this._headerWasOpen[type] = this._isHeaderOpen(type);
      this._updateHeaderLabel(type);
    }
